...
class MovieRepositoryImpl
    ...
    constructor(
        ...
        private fun handleHttpError(
            ...
            val errorMessage = getHttpErrorMessage(e.code(), "filmes")
        ...
        override suspend fun addMovieDetailToFavorites(movieDetail: MovieDetail) {
            try {
                ...
                Log.e(
                    TAG,
                    "addMovieDetailToFavorites: Erro de banco de dados ao " +
                        "adicionar filme ${movieDetail.id} aos favoritos",
                    e,
                )
                throw e
            } catch (e: Exception) {
                Log.e(
                    TAG,
                    "addMovieDetailToFavorites: Erro inesperado ao adicionar filme ${movieDetail.id} aos favoritos",
                    e,
                )
                throw e
            }
        ...
        override suspend fun getMovieDetails(movieId: Int): Result<MovieDetail> {
            return try {
                ...
                handleNetworkErrorForDetails("getMovieDetails", e)
            } catch (e: java.net.SocketTimeoutException) {
                handleTimeoutErrorForDetails("getMovieDetails", e)
            } catch (e: retrofit2.HttpException) {
                handleHttpErrorForDetails("getMovieDetails", e)
            } catch (e: Exception) {
        ...
        private fun handleNetworkErrorForDetails(
            operation: String,
            e: java.net.UnknownHostException,
        ): Result<MovieDetail> {
            Log.e(TAG, "$operation: Sem conexão com a internet", e)
        ...
        private fun handleTimeoutErrorForDetails(
            operation: String,
            e: java.net.SocketTimeoutException,
        ): Result<MovieDetail> {
            Log.e(TAG, "$operation: Timeout na requisição", e)
        ...
        private fun handleHttpErrorForDetails(
            operation: String,
            e: retrofit2.HttpException,
        ): Result<MovieDetail> {
            val errorMessage = getHttpErrorMessage(e.code(), "detalhes do filme")
            Log.e(TAG, "$operation: Erro HTTP ${e.code()}", e)
            return Result.failure(Exception(errorMessage, e))
        }
        ...
        private fun getHttpErrorMessage(
            ...
            return when (code) {
                ...
                404 -> if (context.contains("filme")) "Filme não encontrado." else "Recurso não encontrado."
                429 -> "Muitas requisições. Aguarde um momento e tente novamente."
                500 -> "Erro no servidor. Tente novamente mais tarde."
                else -> "Erro ao buscar $context: $code"
            }
        }
        ...
        override suspend fun getFavoriteMovieDetails(movieId: Int): Result<MovieDetail> {
            return try {
                ...
                Log.e(
                    TAG,
                    "getFavoriteMovieDetails: Erro de banco de dados ao buscar detalhes do filme favorito $movieId",
                    e,
                )
