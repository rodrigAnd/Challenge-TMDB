...
class MovieRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun addMovieDetailToFavorites(movieDetail: MovieDetail) {
            Log.d(
                TAG,
                "addMovieDetailToFavorites: Adicionando filme ${movieDetail.id} aos favoritos com detalhes completos",
            )
            try {
            ...
            return try {
                favoriteDao.getAllFavorites()
                    .map { entities ->
                        ...
                        Log.d(
                            TAG,
                            "getFavoriteMovies: Convertidas ${movies.size} entidades para Movie, IDs: ${movies.map { it.id }}",
                        )
                        movies
                    }
            } catch (e: Exception) {
